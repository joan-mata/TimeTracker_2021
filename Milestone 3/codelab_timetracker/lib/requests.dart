...
Future<void> addTask(int parentId, String name) async {
  var uri = Uri.parse("$baseUrl/addTask?$parentId&$name");
  print(uri);
  final response = await client.get(uri);
  if (response.statusCode == 200) {
    print("statusCode=$response.statusCode");
...
Future<void> addProject(int parentId, String name) async {
  var uri = Uri.parse("$baseUrl/addProject?$parentId&$name");
  final response = await client.get(uri);
  if (response.statusCode == 200) {
    print("statusCode=$response.statusCode");
  } else {
